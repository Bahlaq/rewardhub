...
// GoogleAuth plugin — loaded lazily on native only.
...
// ─── Firestore ────────────────────────────────────────────────────────
// Each of the three lines below is wrapped individually so that a failure
// in one does not prevent the others from initializing. The most common
// failure on iOS < 15.4 is a ReferenceError from structuredClone() being
// called inside initializeFirestore — the polyfill in index.html fixes
// this, but the try-catch ensures the app never hard-crashes even if the
// polyfill is somehow missing.
...
export const firebaseService = {
  ...
  async signInWithGoogle() {
    ...
    if (!googleProvider) throw new Error("GoogleAuthProvider not initialized");
  ...
  async _ensureProfile(fbUser: FirebaseUser, fallbackEmail?: string) {
    ...
    if (!profile) {
      await this.saveUserProfile({
        uid: fbUser.uid,
        email: fbUser.email || fallbackEmail || 'Unknown',
        points: 0,
        claimsToday: 0,
        lastClaimDate: null,
        totalEarned: 0,
  ...
  onAuthChange(cb: (user: FirebaseUser | null) => void) {
  ...
  async saveFcmToken(uid: string, token: string, platform: string) {
    ...
    if (!db) throw new Error('Firestore db is null — saveFcmToken cannot write');
  ...
  async saveUserProfile(profile: UserProfile) {
    if (!db) return;
    try {
      await setDoc(doc(db, getCol(profile.uid), profile.uid), profile, { merge: true });
      localStorage.setItem(`profile_${profile.uid}`, JSON.stringify(profile));
    } catch (e) {
      handleFsError(e, `${getCol(profile.uid)}/${profile.uid}`, false);
    }
  },
  ...
  async checkDailyReset(uid: string) {
    ...
    try {
      ...
      if (d.exists() && d.data().lastBoostDate !== new Date().toDateString()) {
        await setDoc(doc(db, getCol(uid), uid), {
          boostLevel: 1, adsWatchedToday: 0, currentLevelAdCounter: 0,
          lastBoostDate: new Date().toDateString()
        }, { merge: true });
      }
    } catch (e) {
      handleFsError(e, `${getCol(uid)}/${uid}`, false);
    }
  },
  ...
  onClaimsChange(uid: string, cb: (claims: Transaction[]) => void) {
    ...
    return onSnapshot(
      ...
      snap => cb(snap.docs.map(d => {
        const x = d.data();
        return {
          id: d.id, type: 'claim', title: x.offerBrand,
          amount: -x.pointsSpent,
          timestamp: x.timestamp?.toDate?.()?.toISOString() || new Date().toISOString(),
          code: x.code, rewardType: x.code ? 'code' : 'link'
        } as Transaction;
      })),
  ...
  async claimOffer(uid: string, offer: Offer) {
    ...
    await runTransaction(db, async (tx) => {
      ...
      tx.update(userRef, {
        points: Number(u.points) - Number(offer.points),
        claimsToday: (Number(u.claimsToday) || 0) + 1,
        lastClaimDate: new Date().toISOString()
      });
      tx.set(doc(collection(db, 'history')), {
        uid, type: 'claim', points: -Number(offer.points),
        message: `Claimed ${offer.brand}`, timestamp: serverTimestamp()
      });
      tx.set(claimRef, {
        uid, offerId: offer.id, offerBrand: offer.brand,
        code: offer.code || null, url: offer.url,
        pointsSpent: offer.points, timestamp: serverTimestamp()
      });
    });
    return true;
  },
