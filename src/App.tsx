...
const CD_KEY = 'rewardhub_cooldown';
function loadCD() { try { const r = localStorage.getItem(CD_KEY); if (!r) return null; const p = JSON.parse(r); if (Date.now() - p.s >= COOLDOWN_MS) { localStorage.removeItem(CD_KEY); return null; } return p; } catch { return null; } }

const Logo = ({ className }: { className?: string }) => (
  <div className={cn("relative w-full mx-auto", className)}><img src={icon} alt={APP_NAME} className="w-full h-auto object-contain rounded-[2.5rem] shadow-2xl" referrerPolicy="no-referrer" /></div>
);
  ...
  <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-200 px-6 pt-3 z-50" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 0.75rem)' }}>
    <div className="max-w-md mx-auto flex justify-between">
      {[{ id: 'offers', icon: LayoutDashboard, label: 'Rewards' }, { id: 'profile', icon: User, label: 'Profile' }].map(t => (
        <button key={t.id} onClick={() => setActiveTab(t.id)} className={cn("flex flex-col items-center gap-1", activeTab === t.id ? "text-indigo-600" : "text-zinc-400")}>
          <t.icon size={20} strokeWidth={activeTab === t.id ? 2.5 : 2} /><span className="text-[10px] font-medium uppercase tracking-wider">{t.label}</span>
        </button>))}
    </div>
  </nav>
);
...
const WebAdModal = ({ isOpen, onComplete, onCancel, adNumber, totalAds }: { isOpen: boolean; onComplete: () => void; onCancel: () => void; adNumber: number; totalAds: number }) => {
  ...
  return (<div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/90"><div className="w-full max-w-sm bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800">
    ...
    <div className="p-6 text-center"><h3 className="text-lg font-bold text-white mb-2">Sponsored Content</h3><p className="text-sm text-zinc-400 mb-6">{done ? 'Tap Continue.' : 'Please wait.'}</p>
...
const Modal = ({ isOpen, onClose, children }: { isOpen: boolean; onClose: () => void; children: React.ReactNode }) => {
  if (!isOpen) return null;
  return (<div className="fixed inset-0 z-[9999] flex items-center justify-center p-6"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={onClose} className="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm" />
...
export default function App() {
  ...
  // ═══════════════════════════════════════════════════════════════════
  // STATE-BASED notification guard (not a ref!)
  // App Open Ad useEffect DEPENDS on this state.
  // When it transitions from false→true, the useEffect fires.
  // ═══════════════════════════════════════════════════════════════════
  ...
  useEffect(() => { const s = loadCD(); if (s) { const r = COOLDOWN_MS - (Date.now() - s.s); if (r > 0) { setIsCooldown(true); setCdSec(Math.ceil(r / 1000)); startCD(r); } } return () => { if (cdRef.current) clearInterval(cdRef.current); }; }, []);
  function startCD(ms: number) { if (cdRef.current) clearInterval(cdRef.current); const end = Date.now() + ms; setIsCooldown(true); setCdSec(Math.ceil(ms / 1000)); cdRef.current = setInterval(() => { const r = end - Date.now(); if (r <= 0) { clearInterval(cdRef.current!); cdRef.current = null; setIsCooldown(false); setCdSec(0); localStorage.removeItem(CD_KEY); } else setCdSec(Math.ceil(r / 1000)); }, 1000); }
  function activateCD() { localStorage.setItem(CD_KEY, JSON.stringify({ s: Date.now() })); startCD(COOLDOWN_MS); }

  const [selectedCountry, setSelectedCountry] = useState<string>(() => localStorage.getItem('rewardhub_country') || 'All Countries');
  const handleCountry = useCallback((c: string) => { setSelectedCountry(c); localStorage.setItem('rewardhub_country', c); }, []);

  // Auth
  useEffect(() => { const u = firebaseService.onAuthChange(f => { setFirebaseUser(f); if (!f) setIsAuthLoading(false); }); return () => u(); }, []);

  // ═══════════════════════════════════════════════════════════════════
  // NOTIFICATION INIT — runs after auth, sets notificationsDone=true
  // when complete (whether granted, denied, or failed)
  // ═══════════════════════════════════════════════════════════════════
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log('[App] Starting notification init...');
        const { notificationService } = await import('./services/notifications');
        await notificationService.initialize((token: string) => {
          console.log('[App] Saving FCM token...');
          firebaseService.saveFcmToken(uid, token).catch(e => console.error('[App] saveFcmToken error:', e));
        });
        console.log('[App] Notification init complete');
      } catch (err) {
        console.error('[App] Notification init error (non-fatal):', err);
      }
      // ALWAYS mark as done, even if it failed
  ...
  // ONLY fires when notificationsDone becomes true (state dependency).
  // This guarantees the notification permission dialog is fully closed.
  // ═══════════════════════════════════════════════════════════════════
  ...
  useEffect(() => {
    if (!notificationsDone) return; // Wait for notifications to finish
    console.log('[App] Notifications done — showing App Open Ad in 1s');
    const timer = setTimeout(() => showAppOpenAd(), 1000);
    return () => clearTimeout(timer);
  }, [notificationsDone, showAppOpenAd]);

  // App Open Ad — Resume from background (only if notifications already done)
  useEffect(() => {
    const handler = () => {
      if (document.visibilityState === 'hidden') bgRef.current = Date.now();
      else if (document.visibilityState === 'visible' && notificationsDone && bgRef.current > 0 && Date.now() - bgRef.current >= 5000) {
        showAppOpenAd();
      }
    };
  ...
  const displayPts = useMemo(() => Math.max(0, transactions.reduce((a, t) => t.type === 'earn' ? a + t.amount : t.type === 'claim' ? a - t.amount : a, 0)), [transactions]);

  useEffect(() => { const u = onOffersChange(); return () => u(); }, [onOffersChange]);

  // Filter: category → country (UPPERCASE comparison) → search
  ...
  const onWebDone = useCallback(() => { setIsWebAdOpen(false); webAdRef.current?.(true); webAdRef.current = null; }, []);
  const onWebSkip = useCallback(() => { setIsWebAdOpen(false); webAdRef.current?.(false); webAdRef.current = null; }, []);
  ...
  const handleWatchAd = async () => {
    ...
    if (rem <= 0) { await doClaim(); activateCD(); setIsAdRunning(false); if (isNative) showBanner(); return; }
    let done = 0;
    for (let i = 0; i < rem; i++) {
      const n = cp + i + 1, ok = isNative ? await showRewardedAdAndWait() : await showWebAd(n, bl);
    ...
    if (done === rem) { await doClaim(); activateCD(); }
    setIsAdRunning(false); if (isNative && activeTab === 'offers') showBanner();
  };
  const doClaim = async () => { try { const r = await claimBoostReward(); if (r) Toast.show({ text: 'Boost Complete! +100 pts', duration: 'long' }); } catch { Toast.show({ text: "Claim failed.", duration: 'short' }); } };
  ...
  const handleClaimOffer = async (offer: Offer, cost: number) => {
    ...
    try { await firebaseService.claimOffer(user.uid, offer); recordUnlock(offer.id);
      if (!offer.code) Browser.open({ url: offer.url }); else { setConfirmCfg({ title: 'Success!', message: `Code: ${offer.code}`, onConfirm: () => { Clipboard.write({ string: offer.code! }); Toast.show({ text: 'Copied!', duration: 'short' }); } }); setIsConfirmOpen(true); }
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-50 font-sans overflow-hidden">
      <div className="flex-1 overflow-y-auto scroll-smooth relative">
        <Header user={{ ...user, points: displayPts }} />
        <main className="max-w-md mx-auto px-6 py-6 pb-[120px]">
          <AnimatePresence mode="wait">
            {activeTab === 'offers' && <HomeScreen user={{ ...user, points: displayPts }} offers={offers} isLoading={isLoading} searchQuery={searchQuery} setSearchQuery={setSearchQuery} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} categories={categories} filteredOffers={filteredOffers} transactions={transactions} handleWatchAd={handleWatchAd} handleClaimOffer={handleClaimOffer} handleClaimBoostReward={doClaim} isAdRunning={isAdRunning} selectedCountry={selectedCountry} setSelectedCountry={handleCountry} isCooldownActive={isCooldown} cooldownSecondsLeft={cdSec} />}
            {activeTab === 'profile' && (
              <motion.div key="profile" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="bg-white rounded-3xl p-6 border border-zinc-200 shadow-sm text-center">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-8">
                    <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100"><span className="block text-[10px] uppercase font-bold text-zinc-400 mb-1">Points</span><span className="text-lg font-bold text-zinc-900">{displayPts}</span></div>
      ...
      <WebAdModal isOpen={isWebAdOpen} onComplete={onWebDone} onCancel={onWebSkip} adNumber={webAdNum} totalAds={webAdTotal} />
      <Modal isOpen={isPrivacyOpen} onClose={() => setIsPrivacyOpen(false)}>
        ...
        <div className="text-sm text-zinc-600 space-y-3 max-h-[40vh] overflow-y-auto"><p className="font-bold text-zinc-900">Data:</p><p>We collect your email and in-app activity.</p><p className="font-bold text-zinc-900">Deletion:</p><p>Delete anytime from Profile.</p><p className="font-bold text-zinc-900">Ads:</p><p>Google AdMob. We never sell data.</p></div>
      ...
      <Modal isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)}>
        ...
        <button onClick={handleDelete} className="w-full bg-rose-600 text-white py-4 rounded-2xl font-bold active:scale-95 mb-3">Delete</button><button onClick={() => setIsDeleteOpen(false)} className="w-full bg-zinc-100 text-zinc-600 py-4 rounded-2xl font-bold active:scale-95">Cancel</button>
      </Modal>
      <Modal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)}>
        ...
        <button onClick={() => { confirmCfg.onConfirm(); setIsConfirmOpen(false); }} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold active:scale-95 mb-3">Confirm</button><button onClick={() => setIsConfirmOpen(false)} className="w-full bg-zinc-100 text-zinc-600 py-4 rounded-2xl font-bold active:scale-95">Cancel</button>
      </Modal>
    </div>
  );
}
