...
const WebAdSimulatorModal = ({ isOpen, onComplete, onCancel, adNumber, totalAds }: {
  ...
  useEffect(() => { if (!isOpen) return; setTimeLeft(5); setIsFinished(false); const t = setInterval(() => { setTimeLeft(p => { if (p <= 1) { clearInterval(t); setIsFinished(true); return 0; } return p - 1; }); }, 1000); return () => clearInterval(t); }, [isOpen]);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 1.5rem)', paddingBottom: 'calc(env(safe-area-inset-bottom) + 1.5rem)' }}>
      <div className="w-full max-w-sm bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800 shadow-2xl">
        <div className="relative aspect-video bg-zinc-800 flex items-center justify-center"><PlayCircle size={48} className="text-zinc-600 animate-pulse" />
          ...
          <div className="absolute top-4 right-4 bg-black/50 px-3 py-1 rounded-full text-white text-xs font-bold">{isFinished ? 'Done!' : `${timeLeft}s`}</div></div>
        <div className="p-6 text-center"><h3 className="text-lg font-bold text-white mb-2">Watching Sponsored Content</h3><p className="text-sm text-zinc-400 mb-6">{isFinished ? 'Ad complete. Tap Continue.' : 'Please wait.'}</p>
          <button onClick={() => { if (isFinished) onComplete(); else onCancel(); }} className={cn("w-full py-3 rounded-2xl font-bold", isFinished ? "bg-emerald-500 text-white hover:bg-emerald-600" : "bg-zinc-800 text-zinc-500")}>{isFinished ? 'Continue' : 'Skip (no reward)'}</button></div>
      </div>
    </div>
  );
};
...
const SimpleModal = ({ isOpen, onClose, children }: { isOpen: boolean; onClose: () => void; children: React.ReactNode }) => {
  if (!isOpen) return null;
  return (<div className="fixed inset-0 z-[9999] flex items-center justify-center p-6">
...
export default function App() {
  ...
  function startCooldownTicker(durationMs: number) {
  ...
  // App Open Ad
  useEffect(() => { const t = setTimeout(() => showAppOpenAd(), 2000); return () => clearTimeout(t); }, [showAppOpenAd]);
  useEffect(() => {
    const handler = () => {
      if (document.visibilityState === 'hidden') backgroundTimestampRef.current = Date.now();
      else if (document.visibilityState === 'visible' && backgroundTimestampRef.current > 0 && Date.now() - backgroundTimestampRef.current >= 5000) showAppOpenAd();
    };
  ...
  const handleSignIn = async () => {
    ...
    catch (e) { const m = e instanceof Error ? e.message : String(e); Toast.show({ text: m.length > 100 ? m.slice(0, 100) + '...' : m, duration: 'long' }); }
  ...
  const handleDeleteAccount = async () => {
    ...
    try { await firebaseService.deleteUserProfile(user.uid); if (!user.uid.startsWith('local_guest_')) await firebaseService.deleteAccount(); Toast.show({ text: 'Account deleted', duration: 'long' }); }
  ...
  if (!isConfigValid) return (<div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center p-6 text-center"><AlertCircle size={48} className="text-rose-500 mb-4" /><h1 className="text-xl font-bold text-zinc-900 mb-2">Configuration Error</h1></div>);
  ...
  if (!firebaseUser) return (
    <div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center p-8 text-center" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="w-full max-w-sm flex flex-col items-center"><Logo className="max-w-[160px]" /><h1 className="text-3xl font-black tracking-tight text-zinc-900 mt-8 mb-3">Welcome to {APP_NAME}</h1><p className="text-sm text-zinc-500 mb-10 max-w-[280px]">Sign in to start earning points and unlock exclusive rewards.</p>
        <div className="w-full space-y-4">
          <button onClick={handleSignIn} className="w-full bg-white border border-zinc-200 py-4 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-sm hover:shadow-md active:scale-95"><img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="G" className="w-5 h-5" /> Continue with Google</button>
          <button onClick={handleGuestSignIn} className="w-full bg-zinc-900 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-lg hover:bg-zinc-800 active:scale-95"><User size={20} /> Continue as Guest</button>
        </div><p className="mt-12 text-[10px] text-zinc-400 uppercase tracking-widest">v{APP_VERSION}</p></div></div>
  );
  if (!user) return (<div className="h-screen flex flex-col items-center justify-center bg-zinc-50 p-6"><div className="w-12 h-12 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin mb-4" /></div>);
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-50 font-sans selection:bg-indigo-100 overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'offers' && (
              <HomeScreen user={{ ...user, points: Number(displayPoints || 0) }} offers={offers} isLoading={isLoading} searchQuery={searchQuery} setSearchQuery={setSearchQuery}
                ...
                isCooldownActive={isCooldownActive} cooldownSecondsLeft={cooldownSecondsLeft} />
            )}
            {activeTab === 'profile' && (
              <motion.div key="profile" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-6">
                ...
                <div className="space-y-3"><h2 className="text-sm font-bold text-zinc-400 uppercase tracking-widest">Recent History</h2>
                  {transactions.length === 0 ? <div className="bg-white rounded-2xl p-8 border border-dashed border-zinc-200 text-center"><History size={24} className="text-zinc-300 mx-auto mb-2" /><p className="text-xs text-zinc-500">No transactions yet</p></div>
      ...
      <SimpleModal isOpen={isPrivacyModalOpen} onClose={() => setIsPrivacyModalOpen(false)}>
        ...
        <div className="text-sm text-zinc-600 space-y-3 max-h-[40vh] overflow-y-auto"><p className="font-bold text-zinc-900">Data Collection:</p><p>We collect your email and in-app activity for functionality only.</p><p className="font-bold text-zinc-900">Data Deletion:</p><p>Delete your account anytime from Profile settings.</p><p className="font-bold text-zinc-900">Third Parties:</p><p>We use Google AdMob for ads. We never sell your data.</p></div>
      ...
      <SimpleModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
        ...
        <h3 className="text-xl font-black text-zinc-900 mb-2">Delete Account?</h3><p className="text-sm text-zinc-500 mb-8">This is permanent. All data will be lost.</p>
        <div className="flex flex-col gap-3"><button onClick={handleDeleteAccount} className="w-full bg-rose-600 text-white py-4 rounded-2xl font-bold hover:bg-rose-700 active:scale-95">Yes, Delete Everything</button><button onClick={() => setIsDeleteModalOpen(false)} className="w-full bg-zinc-100 text-zinc-600 py-4 rounded-2xl font-bold hover:bg-zinc-200 active:scale-95">Cancel</button></div>
      </SimpleModal>
