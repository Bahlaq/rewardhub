  ...
  <header className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-zinc-100 px-5 pb-2 z-40" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 0.5rem)' }}>
    <div className="max-w-md mx-auto flex justify-between items-center">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-base font-black tracking-tight text-zinc-900 leading-none">{APP_NAME}</h1>
        <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider mt-1">Earn while you play</p></div>
      </div>
...
// ═══════════════════════════════════════════════════════════════════════
// WebAdSimulatorModal — WEB ONLY
// This is the "ad" on web (since AdMob has no web SDK).
// Promise-based: parent opens it, modal resolves the promise on completion.
// Shows which ad in the sequence we're on: "Ad 2 of 3"
// z-[9999] so it's above the banner (z-[1000])
// ═══════════════════════════════════════════════════════════════════════
      ...
      <div className="w-full max-w-sm bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800 shadow-2xl">
        <div className="relative aspect-video bg-zinc-800 flex items-center justify-center">
          ...
          <div className="absolute top-4 right-4 bg-black/50 px-3 py-1 rounded-full text-white text-xs font-bold">
            {isFinished ? 'Complete!' : `${timeLeft}s`}
          </div>
        </div>
        <div className="p-6 text-center">
          <h3 className="text-lg font-bold text-white mb-2">Watching Sponsored Content</h3>
          <p className="text-sm text-zinc-400 mb-6">
            {isFinished ? 'Ad complete. Tap Continue to proceed.' : 'Please wait for the ad to finish.'}
          </p>
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      <div className="w-full max-w-md bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-800 shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-[10px]">
          ...
          : logs.map((l: any) => (
            <div key={l.id} className={cn("p-2 rounded-lg border",
              l.event === 'error' ? "bg-rose-900/20 border-rose-800 text-rose-400" :
              l.event === 'reward' ? "bg-emerald-900/20 border-emerald-800 text-emerald-400" :
              "bg-zinc-800/50 border-zinc-700 text-zinc-300")}>
              <span className="text-zinc-500">{new Date(l.timestamp).toLocaleTimeString()}</span>{' '}
              <span className="font-bold uppercase">[{l.type}:{l.event}]</span> {l.message}
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const webAdResolveRef = useRef<((completed: boolean) => void) | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubP = firebaseService.onProfileChange(uid, p => {
      if (p) setUser(p);
    ...
    const unsubC = firebaseService.onClaimsChange(uid, setFirestoreClaims);
    const unsubH = firebaseService.onHistoryChange(uid, setFirestoreHistory);
  ...
  const transactions = useMemo(() => {
    ...
    return Array.from(new Map(all.map(tx => [tx.id, tx])).values()).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  ...
  const filteredOffers = useMemo(() => offers.filter(o => {
    ...
    return o.brand.toLowerCase().includes(s) || o.description.toLowerCase().includes(s) || o.type.toLowerCase().includes(s);
  ...
  // Banner lifecycle
  useEffect(() => {
    if (activeTab === 'offers' && !isAdRunning && !isWebAdModalOpen) showBanner();
    else hideBanner();
  }, [activeTab, isAdRunning, isWebAdModalOpen, showBanner, hideBanner]);

  // ═══════════════════════════════════════════════════════════════════
  // WEB SIMULATOR — Promise-based wrapper
  // Opens the modal, returns a promise that resolves when user
  // clicks Continue (true) or Skip (false).
  // ═══════════════════════════════════════════════════════════════════
  const showWebSimulatorAndWait = useCallback((adNum: number, total: number): Promise<boolean> => {
    return new Promise(resolve => {
      webAdResolveRef.current = resolve;
      setWebAdNumber(adNum);
      setWebAdTotal(total);
      setIsWebAdModalOpen(true);
    });
  }, []);

  const handleWebAdComplete = useCallback(() => {
    setIsWebAdModalOpen(false);
    webAdResolveRef.current?.(true);
    webAdResolveRef.current = null;
  }, []);

  const handleWebAdCancel = useCallback(() => {
    setIsWebAdModalOpen(false);
    webAdResolveRef.current?.(false);
    webAdResolveRef.current = null;
  }, []);

  // ═══════════════════════════════════════════════════════════════════
  // handleWatchAd — THE CORE FIX FOR THE "REVERSE FLOW" BUG
  //
  // OLD FLOW (broken):
  //   Tap → Modal with 5s countdown (ad shows FIRST as countdown) →
  //   Click Continue → watchAd() → simulateAd() (ANOTHER 3s timer) →
  //   THEN native ad appears (if at all). Result: Countdown before ad.
  //
  // NEW FLOW (correct):
  //   Tap → [Native] AdMob fullscreen IMMEDIATELY → user watches →
  //         user dismisses → recordAdWatch() → next ad or claim
  //   Tap → [Web] Simulator modal (THIS is the "ad") → countdown →
  //         Continue → recordAdWatch() → next ad or claim
  //
  //   Multi-ad: Automatic loop. Ad1 → dismiss → Ad2 → dismiss → ... → Claim
  //   No modal between native ads. No hidden timers. No double counting.
  // ═══════════════════════════════════════════════════════════════════
  const handleWatchAd = async () => {
    ...
    for (let i = 0; i < remaining; i++) {
      ...
      if (isNative) {
        // NATIVE: Real AdMob ad fires IMMEDIATELY. No modal.
        // showRewardedAdAndWait() resolves ONLY after user dismisses the ad.
        addLog('rewarded', 'load', `Starting ad ${adNum}/${adsNeeded}`);
        completed = await showRewardedAdAndWait();
      } else {
        // WEB: Simulator modal IS the ad. Opens, countdown runs, user clicks Continue.
        completed = await showWebSimulatorAndWait(adNum, adsNeeded);
      }

      if (!completed) {
        Toast.show({ text: 'Ad was not completed. Try again.', duration: 'short' });
        break;
      }

      // Record this ad watch in Firestore
      const result = await recordAdWatch();
      if (result) {
        adsCompleted++;
        if (adNum < adsNeeded) {
          Toast.show({ text: `Ad ${adNum}/${adsNeeded} complete!`, duration: 'short' });
          // Brief pause between ads so the toast is visible
          await new Promise(r => setTimeout(r, 800));
        }
      }
    }

    // After loop: if all ads were completed, auto-claim
    if (adsCompleted === remaining) {
      await handleClaimBoostReward();
    }
  ...
  const handleClaimBoostReward = async () => {
    try {
      ...
      if (result) {
        ...
        Toast.show({ text: `🎉 Boost Level ${result.boostLevel - 1} Complete! +100 pts`, duration: 'long' });
      }
    } catch (error) {
  ...
  const handleClaimOffer = async (offer: Offer, cost: number) => {
    if (!user) return;
    if (user.points < cost) {
      setConfirmConfig({ title: 'Not Enough Points', message: `You need ${cost - user.points} more points. Watch ads to earn progress.`, onConfirm: handleWatchAd });
      setIsConfirmModalOpen(true); return;
    }
    try {
      await firebaseService.claimOffer(user.uid, offer);
      if (!offer.code) { Browser.open({ url: offer.url }); }
      else { setConfirmConfig({ title: 'Success!', message: `Code: ${offer.code}. Saved to history.`, onConfirm: () => { Clipboard.write({ string: offer.code! }); Toast.show({ text: 'Copied!', duration: 'short' }); } }); setIsConfirmModalOpen(true); }
    } catch { Toast.show({ text: "Failed to claim.", duration: 'long' }); }
  };
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      Toast.show({ text: `Sign-In failed: ${m.slice(0, 60)}`, duration: 'long' });
  ...
  const handleGuestSignIn = async () => {
    ...
    try { const f = await firebaseService.signInAnonymously(); setFirebaseUser(f); }
      ...
      setFirebaseUser({ uid: id, isAnonymous: true } as any);
  ...
  const renderMain = () => {
    ...
    return (
      <div className="h-screen flex flex-col bg-zinc-50 font-sans selection:bg-indigo-100 overflow-hidden">
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'offers' && (
                <HomeScreen user={{ ...user, points: Number(displayPoints || 0) }}
                  ...
                  isBannerVisible={isBannerVisible && !isNative} isAdRunning={isAdRunning} />
              )}
              {activeTab === 'profile' && (
                  ...
                  <div className="bg-white rounded-3xl p-6 border border-zinc-200 shadow-sm text-center">
                    ...
                    <div className="mt-6 pt-6 border-t border-zinc-100">
                      <button onClick={() => setIsDebugModalOpen(true)} className="w-full flex items-center justify-between p-4 bg-zinc-50 rounded-2xl text-left hover:bg-zinc-100 transition-colors">
                        <div className="flex items-center gap-3"><Terminal size={18} className="text-zinc-400" /><span className="text-sm font-bold text-zinc-700">Debugger</span></div><ChevronRight size={18} className="text-zinc-400" />
                      </button>
                    </div>
                  </div>
                    ...
                    : transactions.slice(0, 20).map(tx => (
                      <div key={tx.id} className="bg-white rounded-2xl p-4 border border-zinc-200 shadow-sm flex justify-between items-center">
                        ...
                        <div className="text-right">
                          ...
                          {tx.code && <button onClick={async () => { await Clipboard.write({ string: tx.code! }); Toast.show({ text: 'Copied!', duration: 'short' }); }} className="flex items-center gap-1 text-[10px] font-mono bg-zinc-100 px-2 py-0.5 rounded text-zinc-600 border border-zinc-200 hover:bg-zinc-200">{tx.code}<Copy size={10} /></button>}
                        </div>
                      </div>
                    ))}
                    <div className="h-40" />
        ...
        {/* Web-only Ad Simulator — NEVER shown on native */}
        ...
        <SimpleModal isOpen={isPrivacyModalOpen} onClose={() => setIsPrivacyModalOpen(false)}>
          ...
          <div className="text-sm text-zinc-600 space-y-3 max-h-[40vh] overflow-y-auto pr-2">
            <p className="font-bold text-zinc-900">Data Collection:</p><p>We collect your email and in-app activity for functionality.</p>
            <p className="font-bold text-zinc-900">Data Deletion:</p><p>Delete your account anytime via Settings.</p>
            <p className="font-bold text-zinc-900">Third Parties:</p><p>We use Google AdMob for ads. We do not sell your data.</p>
          </div>
          <button onClick={() => setIsPrivacyModalOpen(false)} className="w-full mt-6 bg-indigo-600 text-white py-4 rounded-2xl font-bold hover:bg-indigo-700 transition-all active:scale-95">Got it</button>
        </SimpleModal>
        ...
        <SimpleModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
          ...
          <p className="text-sm text-zinc-500 mb-8">This is permanent. All data will be lost.</p>
          <div className="flex flex-col gap-3">
            <button onClick={handleDeleteAccount} className="w-full bg-rose-600 text-white py-4 rounded-2xl font-bold hover:bg-rose-700 active:scale-95">Yes, Delete</button>
            <button onClick={() => setIsDeleteModalOpen(false)} className="w-full bg-zinc-100 text-zinc-600 py-4 rounded-2xl font-bold hover:bg-zinc-200 active:scale-95">Cancel</button>
          </div>
        </SimpleModal>
        ...
        <SimpleModal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)}>
          ...
          <h3 className="text-xl font-black text-zinc-900 mb-2">{confirmConfig.title}</h3>
          <p className="text-sm text-zinc-500 mb-8">{confirmConfig.message}</p>
          <div className="flex flex-col gap-3">
            <button onClick={() => { confirmConfig.onConfirm(); setIsConfirmModalOpen(false); }} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold hover:bg-indigo-700 active:scale-95">Confirm</button>
            <button onClick={() => setIsConfirmModalOpen(false)} className="w-full bg-zinc-100 text-zinc-600 py-4 rounded-2xl font-bold hover:bg-zinc-200 active:scale-95">Cancel</button>
          </div>
        </SimpleModal>
      </div>
    );
