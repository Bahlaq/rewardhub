...
export default function App() {
  ...
  const handleWatchAd = async () => {
    ...
    if (remaining <= 0) { await handleClaimBoostReward(); setIsAdRunning(false); if (isNative) showBanner(); return; }
  ...
  const handleClaimOffer = async (offer: Offer, cost: number) => {
    if (!user) return;
    if (user.points < cost) { setConfirmConfig({ title: 'Not Enough Points', message: `Need ${cost - user.points} more pts. Watch ads to earn.`, onConfirm: handleWatchAd }); setIsConfirmModalOpen(true); return; }
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      Toast.show({ text: m.length > 100 ? m.slice(0, 100) + '...' : m, duration: 'long' });
  ...
  if (!firebaseUser) return (
    <div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center p-8 text-center" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="w-full max-w-sm flex flex-col items-center">
        ...
        <h1 className="text-3xl font-black tracking-tight text-zinc-900 mt-8 mb-3">Welcome to {APP_NAME}</h1>
        <p className="text-sm text-zinc-500 mb-10 max-w-[280px]">Sign in to start earning points and unlock exclusive rewards.</p>
        <div className="w-full space-y-4">
          <button onClick={handleSignIn} className="w-full bg-white border border-zinc-200 py-4 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-sm hover:shadow-md transition-all active:scale-95">
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="w-5 h-5" />
            Continue with Google
          </button>
          <button onClick={handleGuestSignIn} className="w-full bg-zinc-900 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-lg hover:bg-zinc-800 transition-all active:scale-95">
    ...
    <div className="h-screen flex flex-col bg-zinc-50 font-sans selection:bg-indigo-100 overflow-hidden">
      ...
      <SimpleModal isOpen={isPrivacyModalOpen} onClose={() => setIsPrivacyModalOpen(false)}>
        ...
        <div className="text-sm text-zinc-600 space-y-3 max-h-[40vh] overflow-y-auto">
          <p className="font-bold text-zinc-900">Data Collection:</p><p>We collect your email and in-app activity for functionality only.</p>
          <p className="font-bold text-zinc-900">Data Deletion:</p><p>Delete your account and all data anytime from your Profile settings.</p>
          <p className="font-bold text-zinc-900">Third Parties:</p><p>We use Google AdMob for advertisements. We never sell your data.</p>
        </div>
      ...
      <SimpleModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
        ...
        <h3 className="text-xl font-black text-zinc-900 mb-2">Delete Account?</h3><p className="text-sm text-zinc-500 mb-8">This action is permanent. All your points and history will be deleted forever.</p>
