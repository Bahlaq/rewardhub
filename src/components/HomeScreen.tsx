...
// ═══════════════════════════════════════════════════════════════════════
// Country matching — handles ALL Firestore formats:
//   countries: ["UAE", "JORDAN"]     → array of uppercase strings
//   countries: "Jordan"              → single string
//   countries: "GLOBAL"              → global
//   countries: null / undefined / [] / "" → global (show everywhere)
//
// Comparison is ALWAYS case-insensitive (both sides → uppercase)
// ═══════════════════════════════════════════════════════════════════════
export function offerMatchesCountry(offer: Offer, selectedCountry: string): boolean {
  ...
  if (Array.isArray(c)) {
    // Array: check if ANY element matches or is GLOBAL
    return c.some(item => {
      const upper = String(item).toUpperCase();
      return upper === selected || upper === 'GLOBAL';
    });
  }
  ...
  // Unknown type → treat as global
  return true;
}

// 48-hour unlock expiration
...
export function isOfferUnlocked(offerId: string, transactions: Transaction[], brand: string): boolean {
  ...
  return ts ? Date.now() - ts < UNLOCK_MS : false;
}
export function recordUnlock(offerId: string) {
  const u = getUnlocks(); u[offerId] = Date.now();
  const now = Date.now();
  Object.keys(u).forEach(k => { if (now - u[k] >= UNLOCK_MS) delete u[k]; });
  localStorage.setItem(UNLOCK_KEY, JSON.stringify(u));
}
export function getUnlockTimeRemaining(offerId: string): string | null {
  const ts = getUnlocks()[offerId];
  if (!ts) return null;
  const r = UNLOCK_MS - (Date.now() - ts);
  if (r <= 0) return null;
  return `${Math.floor(r / 3600000)}h ${Math.floor((r % 3600000) / 60000)}m left`;
}
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)}
        ...
        <span className="flex-1 text-left truncate">{selectedCountry}</span>
        <ChevronDown size={16} className={cn("text-zinc-400 transition-transform shrink-0", isOpen && "rotate-180")} />
      </button>
      {isOpen && (
        <>
...
const OfferCard = ({ offer, onClaim, user, isUnlocked, claimedCode, unlockTimeLeft }: {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      {isUnlocked && <div className="absolute top-3 left-3 z-20 flex items-center gap-1.5 bg-emerald-500 text-white px-2 py-1 rounded-lg font-bold text-[10px] shadow-lg uppercase tracking-wider"><CheckCircle2 size={12} /> {unlockTimeLeft || 'Unlocked'}</div>}
      ...
      <div className="p-4">
        ...
        {isUnlocked ? (
          <div className="space-y-3">
            {(claimedCode || offer.code) && <div className="flex items-center gap-2"><div className="flex-1 bg-zinc-100 px-3 py-2 rounded-xl font-mono text-sm font-bold text-zinc-700 border border-zinc-200 truncate">{claimedCode || offer.code}</div>
              <button onClick={async () => { await Clipboard.write({ string: claimedCode || offer.code! }); await Toast.show({ text: 'Copied!', duration: 'short' }); }} className="p-2 bg-indigo-50 text-indigo-600 rounded-xl"><Copy size={18} /></button></div>}
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onClaim(offer, offer.points)} disabled={isLocked}
              className={cn("px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2", isLocked ? "bg-zinc-100 text-zinc-400 cursor-not-allowed" : "bg-indigo-600 text-white active:scale-95")}>
              {isLocked ? <><Clock size={14} /> Locked</> : <><Zap size={14} /> Unlock</>}
            </button>
          </div>
        )}
      </div>
...
export const HomeScreen = ({
  ...
  const bl = isNewDay ? 1 : (Number(user.boostLevel) || 1);
  const cp = isNewDay ? 0 : (Number(user.currentLevelAdCounter) || 0);
  const needed = bl;
  const complete = cp >= needed;
  const pct = Math.min((cp / Math.max(needed, 1)) * 100, 100);
  const title = bl === 1 ? 'First Boost' : bl === 2 ? 'Second Boost' : bl === 3 ? 'Third Boost' : `${bl}th Boost`;
  const cdDisp = `${Math.floor(cooldownSecondsLeft / 60)}:${String(cooldownSecondsLeft % 60).padStart(2, '0')}`;
    ...
    <motion.div key="offers" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-5">
          ...
          {categories.map(cat => (
            <button key={cat} onClick={() => setSelectedCategory(cat)}
              className={cn("px-4 py-2 rounded-xl text-xs font-bold capitalize border shadow-sm active:scale-95 whitespace-nowrap",
                selectedCategory === cat ? "bg-indigo-600 border-indigo-700 text-white" : "bg-white border-zinc-200 text-zinc-600")}>{cat}</button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-3xl p-6 text-white shadow-xl shadow-indigo-200 overflow-hidden relative">
        <div className="relative z-10">
          ...
          <p className="text-indigo-100 text-xs mb-4">
            {isCooldownActive ? `Next level in ${cdDisp}` : isAdRunning ? 'Playing ads...' : complete ? `All ${needed} ads done! Claim 100 pts.` : `Progress: ${cp}/${needed} ads (+100 pts)`}
          </p>
          <div className="flex items-center gap-3">
            {isCooldownActive ? <div className="bg-white/10 border border-white/20 px-5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 text-white/70"><Timer size={16} className="animate-pulse" /> {cdDisp}</div>
            ...
            : <button onClick={handleWatchAd} className="bg-white text-indigo-600 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 active:scale-95"><PlayCircle size={16} /> Watch Ad ({cp}/{needed})</button>}
      ...
      <div className="grid gap-4">
        ...
        : filteredOffers.length > 0 ? filteredOffers.map(o => {
          const unlocked = isOfferUnlocked(o.id, transactions, o.brand);
          return <OfferCard key={o.id} offer={o} onClaim={handleClaimOffer} user={user} isUnlocked={unlocked}
            claimedCode={transactions.find(t => t.title === o.brand)?.code} unlockTimeLeft={getUnlockTimeRemaining(o.id)} />;
